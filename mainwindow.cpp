...
MainWindow::MainWindow(QWidget *parent): QMainWindow(parent), ui(new Ui::MainWindow){
    ...
    // aquire available ports
    QList<QSerialPortInfo> serialPortInfos = QSerialPortInfo::availablePorts();
    // output the number of available ports
    ...
    model->setHorizontalHeaderLabels({"","功能码", "起始地址", "单位长度","数据类型","变比","数据库变量", "当前值","实际值"});
    model->setItem(0, 2, new QStandardItem("0x0000"));
    ...
    ui->tableView->setIndexWidget(model->index(0, 1), func);
    ...
    ui->tableView->setIndexWidget(model->index(0, 4), dat);
...
void MainWindow::_changeText(){
    switch (this->ui->comboBox_func->currentText().toInt()) {
    case 1:
        ui->label_x->setText(QStringLiteral("起始地址："));
        ui->label_y->setText(QStringLiteral("线圈数量："));
        break;
...
void MainWindow::_click_PlusButton(){
    ...
    ui->tableView->setIndexWidget(model->index(index, 1), cmb);
    // add dataType
    QComboBox* dat = new QComboBox();
    list_dataType.append(dat);
    dat->addItems({"整型","小数","布尔型","无符号整数"});
    dat->setCurrentIndex(list_dataType.at(index-1)->currentIndex());
    ui->tableView->setIndexWidget(model->index(index, 4), dat);
    // add minus button
    QPushButton*pub = new QPushButton();
    list_minusButton.append(pub);
    pub->setText("-");
    ...
    // add default of 单位长度
    int unit = model->item(index-1,3)->text().toInt();
    QString str = QString::number(unit);
    model->setItem(index, 3, new QStandardItem(str));
    // add default of 起始地址
    str = QString::number(unit,16);
    int x = model->item(index-1,2)->text().toInt(nullptr,16);
    int y = str.toInt(nullptr,16);
    str =  QString("0x%1").arg(x+y,4,16,QLatin1Char('0'));//value为int型或char型都可
    model->setItem(index,2,new QStandardItem(str));
    // add default of 变比
    QString str2 = QString("%1").arg(model->item(index-1,5)->text().toFloat());
    model->setItem(index, 5, new QStandardItem(str2));
    // reset plusButton
...
void append_CRC(QByteArray& frame){
    ...
    unsigned uIndex ; // index of CRC look-up table
    while (pushMsg!=frame.end())
    {
